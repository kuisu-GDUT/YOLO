...
# voc数据集根目录以及版本
voc_root = r"E:\kuisu\Datasets\DLH101"
voc_version = "V1"
...
# 转换后的文件保存目录
save_file_root = r"E:\kuisu\Datasets\DLH101\V1\my_yolo_dataset"

# label标签对应json文件
label_json_path = './DLH101_classes.json'
...
def translate_info(file_names: list, save_root: str, class_dict: dict, train_val='train'):
    ...
    yoloFormatFile = open("data/yoloFormat_{}_label.txt".format(train_val),"a+")
    for file in tqdm(file_names, desc="translate {} file...".format(train_val)):
        yoloFormat = []
        # 检查下图像文件是否存在
        img_path = os.path.join(voc_images_path, file + ".jpg")
        assert os.path.exists(img_path), "file:{} not exist...".format(img_path)
        yoloFormat.append(img_path)
        ...
        with open(os.path.join(save_txt_path, file + ".txt"), "w") as f:
            for index, obj in enumerate(data["object"]):
                ...
                obj = [str(i) for i in [xmin,ymin,xmax,ymax,class_index]]
                yoloFormat.append(",".join(obj))
                if index == 0:
                    f.write(" ".join(info))
                else:
                    f.write("\n" + " ".join(info))
        yoloFormatFile.write(" ".join(yoloFormat)+"\n")

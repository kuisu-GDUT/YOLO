...
if __name__ == '__main__':
    # 调试用，依次取出数据看看是否正确
    dataset_dir = r"E:\kuisu\Datasets\voc\VOC2012\voc2012_forYolov1"
    dataset = MyDataset(dataset_dir)
    dataloader = DataLoader(dataset, 1)
    for i, dataset1 in enumerate(dataloader):
        image, label = dataset1
        print("image shape: {}, labels shape: {}".format(image.shape,label.shape))

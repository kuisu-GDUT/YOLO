...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    tcls, tbox, indices, anchors = build_targets(p, targets, model)  # targets->cls, box, imgIdex,anchorsIdx
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            # 对应匹配到正样本的预测信息
            ps = pi[b, a, gj, gi]  # prediction subset corresponding to targets->[n, 25]

            # GIoU
            pxy = ps[:, :2].sigmoid()#sigmoid(pxy)
            pwh = ps[:, 2:4].exp().clamp(max=1E3) * anchors[i]#pwh^exp()
            ...
            if model.nc > 1:  # cls loss (only if multiple classes)
                ...
                lcls += BCEcls(ps[:, 5:], t)  # BCE->二元交叉熵损损失函数

            # Append targets to text file
            # with open('targets.txt', 'a') as file:
            #     [file.write('%11.5g ' * 4 % tuple(x) + '\n') for x in torch.cat((txy[i], twh[i]), 1)]

        lobj += BCEobj(pi[..., 4], tobj)  # obj loss->该cell是否含有物体损失
